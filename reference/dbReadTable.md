...
[`DBI::dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.md)
with `SELECT * FROM <name>`.

An empty table is returned as a data frame with zero rows.

The presence of [rownames](https://dbi.r-dbi.org/reference/rownames.md)
depends on the `row.names` argument, see
[`DBI::sqlColumnToRownames()`](https://dbi.r-dbi.org/reference/rownames.md)
for details:
