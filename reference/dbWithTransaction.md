...
Given that
[transactions](https://dbi.r-dbi.org/reference/transactions.md) are
...
[`dbBegin()`](https://dbi.r-dbi.org/reference/transactions.md) before
...
is all taken care of. The special function `dbBreak()` allows an early
exit with rollback, it can be called only inside `dbWithTransaction()`.
...
or invalid or if
[`DBI::dbBegin()`](https://dbi.r-dbi.org/reference/transactions.md) has
...
[`DBI::dbCommit()`](https://dbi.r-dbi.org/reference/transactions.md). If
the code raises an error, the transaction is instead aborted with
[`DBI::dbRollback()`](https://dbi.r-dbi.org/reference/transactions.md),
