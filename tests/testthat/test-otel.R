test_that("OpenTelemetry tracing works", {
  ...
  expect_equal(traces[[3L]]$name, "ALTER TABLE `mtcars`")
  expect_equal(traces[[3L]]$attributes$db.collection.name, "`mtcars`")
  expect_equal(traces[[3L]]$attributes$db.operation.name, "ALTER TABLE")
  ...
  expect_equal(traces[[5L]]$attributes$db.query.text, "<redacted>")
  expect_equal(traces[[6L]]$name, "SELECT mtcars")
  expect_equal(traces[[6L]]$attributes$db.query.text, "SELECT COUNT(*) FROM mtcars WHERE cyl = ?")
