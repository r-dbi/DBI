...
See `vignette("DBI", package = "DBI")` and `vignette("DBI-advanced", package = "DBI")`
...
This allows existing code to be used with Arrow,
and it allows new code to be written that is more efficient and more flexible than code that uses R's data frames.

The interface is built around the {nanoarrow} R package,
with `nanoarrow::as_nanoarrow_array` and `nanoarrow::as_nanoarrow_array_stream` as fundamental data structures.
...
The fallback is not improving performance,
but it allows existing code to be used with Arrow before switching to a backend with native Arrow support.
Backends with native support, like the [adbi](https://adbi.r-dbi.org/) package,
implement the new generics and classes for direct support and improved performance.
...
See the [adbi](https://adbi.r-dbi.org/) package for a backend with native Arrow support,
