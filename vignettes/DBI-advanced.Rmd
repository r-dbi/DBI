...
safe_params <- list( dbQuoteString(con,'G') )
...
dbBind(res, list( dbQuoteString(con,'G') ))
df <- dbFetch(res)
dbBind(res, list( dbQuoteString(con,'PG') ))
df <- dbFetch(res)
dbClearResult(res)
...
```{r bind_quotestring}
res <- dbSendQuery(con, "SELECT * FROM film WHERE rating = ?")
dbBind(res, list(lapply(c("G", "PG"), dbQuoteString,conn=con)))
df <- dbFetch(res)
dbClearResult(res)
dbDisconnect(con)
```
