...
#' - `?name1`, `?name2`, ...: values are given as named arguments or a named list,
...
#'   Variables must start with a question mark and can be any valid R identifier,
#'   i.e. it must start with a letter or `.`, and be followed by a letter, digit, `.` or `_`.
#' @param ...,.dots Values (for `...`) or a list (for `.dots`) to interpolate into a string.
#'   Names are required if  `sql` uses the `?name` syntax for placeholders.
#'   All values will be first escaped with [dbQuoteLiteral()] prior to interpolation
#'   to protect against SQL injection attacks.
#'   Arguments created by [SQL()] or [dbQuoteIdentifier()] remain unchanged.
#' @return The `sql` query with the values from `...` and `.dots` safely embedded.
#'
#' @template methods
