...
and more see
[`vignette("DBI-advanced", package = "DBI")`](https://dbi.r-dbi.org/articles/DBI-advanced.md).
...
[`dbConnect()`](https://dbi.r-dbi.org/reference/dbConnect.md) function.
The first argument to the function is the driver for the DBMS you are
connecting to. In the example above we are connecting to a MariaDB
instance, so we use the
...
[`dbListFields()`](https://dbi.r-dbi.org/reference/dbListFields.md). It
takes as arguments a database connection and a table name and returns a
character vector of the column names in order.
...
The function
[`dbReadTable()`](https://dbi.r-dbi.org/reference/dbReadTable.md) reads
an entire table and returns it as a data frame. It is equivalent to the
SQL query `SELECT * FROM <name>`. The columns of the returned data frame
share the same names as the columns in the table. DBI and the database
backends do their best to coerce data to equivalent R data types.
...
[`dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.md) takes a
SQL `SELECT` query to execute and returns a data frame. Below is a basic
query that specifies the columns we require (`film_id`, `title` and
`description`) and which rows (records) we are interested in. Here we
retrieve films released in the year 2006.
