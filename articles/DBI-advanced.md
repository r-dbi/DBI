...
[`vignette("DBI", package = "DBI")`](https://dbi.r-dbi.org/articles/DBI.md)
for a more basic tutorial covering connecting to DBMS and executing
simple queries.

## How to run more complex queries using DBI

[`dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.md) works by
calling a number of functions behind the scenes. If you need more
control you can manually build your own query, retrieve results at your
selected rate, and release the resources involved by calling the same
functions.
...
- [`dbFetch()`](https://dbi.r-dbi.org/reference/dbFetch.md) is called
  with the `result` object returned by
  [`dbSendQuery()`](https://dbi.r-dbi.org/reference/dbSendQuery.md). It
  also accepts an argument specifying the number of rows to be returned,
  e.g. `n = 200`. If you want to fetch all the rows, use `n = -1`.
...
[`dbFetch()`](https://dbi.r-dbi.org/reference/dbFetch.md) is used with
the `n = X` argument, specifying how many rows to return on each
iteration. Again, we call
[`dbClearResult()`](https://dbi.r-dbi.org/reference/dbClearResult.md) at
...
[`dbSendQuery()`](https://dbi.r-dbi.org/reference/dbSendQuery.md) can be
used with parameterized SQL queries. DBI supports two ways to avoid SQL
injection attacks from user-supplied parameters: quoting and
...
[`dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.md) can be
used. It takes a list and its members are substituted in order for the
...
values, [`dbBind()`](https://dbi.r-dbi.org/reference/dbBind.md) is used.
There are two ways to use
[`dbBind()`](https://dbi.r-dbi.org/reference/dbBind.md). Firstly, it can
be used multiple times with same query.
...
Secondly, [`dbBind()`](https://dbi.r-dbi.org/reference/dbBind.md) can be
used to execute the same statement with multiple values at once.
...
[`dbExecute()`](https://dbi.r-dbi.org/reference/dbExecute.md) passes the
SQL statement to the DBMS for execution and returns the number of rows
affected.
[`dbSendStatement()`](https://dbi.r-dbi.org/reference/dbSendStatement.md)
...
In actuality,
[`dbExecute()`](https://dbi.r-dbi.org/reference/dbExecute.md) is a
...
undone with
[`dbRollback()`](https://dbi.r-dbi.org/reference/transactions.md). The
example below updates two tables and ensures that either both tables are
updated, or no changes are persisted to the database and an error is
thrown.
...
[`dbCommit()`](https://dbi.r-dbi.org/reference/transactions.md) on
success and call
[`dbRollback()`](https://dbi.r-dbi.org/reference/transactions.md) if an
error is thrown.
...
[`vignette("spec", package = "DBI")`](https://dbi.r-dbi.org/articles/spec.md).
