...
Take the time now to head over to the `DBItest` vignette at
...
[`dbUnloadDriver()`](https://dbi.r-dbi.org/reference/dbDriver.md)
method.
...
[`dbConnect()`](https://dbi.r-dbi.org/reference/dbConnect.md) which
...
Next, implement [`show()`](https://rdrr.io/r/methods/show.html) and
...
[`dbFetch()`](https://dbi.r-dbi.org/reference/dbFetch.md) method. This
needs to take a result set and (optionally) number of records to return,
and create a dataframe. Mapping R’s data types to those of your database
may require a custom implementation of the
[`dbDataType()`](https://dbi.r-dbi.org/reference/dbDataType.md) method
for your connection class:
...
[`dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.md) to send
a query to the database, retrieve results if available and then clean
up. Spend some time now making sure this works with an existing
...
- [`dbReadTable()`](https://dbi.r-dbi.org/reference/dbReadTable.md): a
  simple wrapper around `SELECT * FROM table`. Use `dbQuoteIdentifer()`
  to safely quote the table name and prevent mismatches between the
  names allowed by R and the database.
...
- [`dbIsValid()`](https://dbi.r-dbi.org/reference/dbIsValid.md) returns
  if a connection or a result set is open (`TRUE`) or closed (`FALSE`).
  All further methods in this section are valid for result sets only.
...
- [`dbBind()`](https://dbi.r-dbi.org/reference/dbBind.md) allows using
  parametrised queries. Take a look at
